...
- **Chat Assistant**: Conversational Dioxus tab integrated with rule-based task shortcuts and Google Gemini API (via local agy CLI and transcript log parsing) with real-time orchestrator system context injection (projects, tasks, and service status) and automated task extraction (`[CREATE_TASK: Title | Body]`).
